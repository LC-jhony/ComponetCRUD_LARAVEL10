@if (session()->has('info'))
    <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 6000)" x-show="show"
        class="fixed flex items-center gap-3 px-6 py-4 text-sm text-white bg-blue-600 rounded-lg bottom-3 right-3">
...
@if (session()->has('danger'))
    <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 6000)" x-show="show"
        class="fixed flex items-center gap-3 px-6 py-4 text-sm text-white bg-red-600 rounded-lg bottom-3 right-3">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
            <path fill-rule="evenodd"
...
@if (session()->has('success'))
    <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 6000)" x-show="show"
        class="fixed flex items-center gap-3 px-6 py-4 text-sm text-white bg-green-600 rounded-lg bottom-3 right-3">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
            <path fill-rule="evenodd"
...
@if (session()->has('warning'))
    <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 6000)" x-show="show"
        class="fixed flex items-center gap-3 px-6 py-4 text-sm text-white bg-yellow-600 rounded-lg bottom-3 right-3">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
            <path fill-rule="evenodd"
...
@if (session()->has('dark'))
    <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 6000)" x-show="show"
        class="fixed flex items-center gap-3 px-6 py-4 text-sm text-white bg-gray-600 rounded-lg bottom-3 right-3">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
            <path
